...
export class BillableHandler {
  ...
  async handle(record: DynamoDBRecord) {
    ...
    const data = (await
      (await 
        (await 
          this.validateEligibility({ record })
  ...
  private async createInvoice(ctx: Context) {
    return (await this.easybillFacade.createInvoice(ctx.companyProfile!, ctx.billable!))
      .mapRight((invoice) => ({...ctx, invoice}));
  }

  private async retrieveCompanyProfile(ctx: Context) {
    return (await this.companyFacade.getProfile(ctx.billable!.companyId))
      .mapRight((companyProfile) => ({ ...ctx, companyProfile }));
  }

  private toBillable(ctx: Context) {
    return mapperToBillable(ctx.record).mapRight((billable) => ({ ...ctx, billable }));
  }
