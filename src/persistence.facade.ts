...
export class PersistenceFacade {
  ...
  async markAsCreated(billable: Billable, invoice: Invoice): Promise<Either<ContextAwareException, null>> {
    const params = {
      ...
      UpdateExpression: "SET process_status = :newStatus, external_id = :newExternalId, updated_at = :newUpdatedAt",
    ...
    try {
      ...
      return Either.left(PersistenceLayerException.create(JSON.stringify(error)));
    }
  }
}
